...
1. **Merge `val` into `train`** and create a stratified 80/20 train/validation split (official val set is unusable at 16 images).
2. **Resize all images** to a fixed input size (e.g., 224 × 224) given the wide dimension variance.
3. **Normalize color mode** — convert RGB files to grayscale or replicate to 3 channels for pretrained models.
4. **Handle class imbalance** — use weighted loss, weighted sampling, or report precision/recall/F1 rather than accuracy alone.
5. **Deduplicate before splitting** — remove or flag the 30 exact-duplicate groups to avoid optimistic validation scores.
6. **Apply train-only augmentation** (flip, rotation, brightness jitter) to improve generalization given the relatively modest dataset size.
7. **Evaluate on the held-out test set** (624 images) for final performance reporting.

---

## 6. Implications for the Presentation

These findings directly support the "Descriptive summary of training data" rubric section:

- **5,856 pediatric chest X-rays**, binary labeled, heavily imbalanced (~73% pneumonia).
- **Validation set is inadequate** — motivates the project's custom split strategy.
- **Images are high-resolution grayscale JPEGs** with variable dimensions, requiring standard CNN preprocessing.
- **Data quality is generally good** (no corruption, no cross-split leakage), with manageable duplicate and outlier issues.
